...
export default function RolePage() {
  ...
  const handleSave = async () => {
    ...
    setSaving(true)
    try {
      if (modal.mode === 'add') await api.post('/roles', { name: form.name })
      else await api.put(`/roles/${modal.item.ID}`, { name: form.name })
      fetchData()
      setModal({ open: false })
    } catch (err) { console.error(err) }
      ...
      <div className="max-w-4xl mx-auto">
        <div className="flex items-center justify-between mb-5">
          <div>
            ...
            <p className="text-gray-500 text-sm">Kelola role pengguna</p>
          </div>
        ...
        <div className="mb-4">
          <input type="text" placeholder="Cari role..." value={search} onChange={e => setSearch(e.target.value)} className="w-full max-w-xs px-4 py-2 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-emerald-400" />
        </div>
        <Table columns={columns} data={filtered} loading={loading} />
        <Modal isOpen={modal.open} onClose={() => setModal({ open: false })} title={modal.mode === 'add' ? 'Tambah Role' : 'Edit Role'}>
          <div className="space-y-4">
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Nama Role</label>
              <input type="text" value={form.name} onChange={e => setForm({ name: e.target.value })} placeholder="Contoh: Owner, Kasir, Admin" className="w-full px-4 py-2.5 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-emerald-400" onKeyDown={e => e.key === 'Enter' && handleSave()} />
            </div>
