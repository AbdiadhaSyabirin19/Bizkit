    ...
    return salesData.filter(sale => {
      ...
      return items.some(item => {
        if (activeTab === 'product') {
          return String(item.ProductID || item.product_id) === selectedID
        } else {
          const catID = String(item.Product?.CategoryID || item.product?.category_id || item.Product?.category_id || '')
    ...
    filteredSales.forEach(sale => {
      ...
      ;(sale.Items || sale.items || []).forEach(item => {
        if (activeTab === 'product' && String(item.ProductID || item.product_id) === selectedID) {
          ...
          const catID = String(item.Product?.CategoryID || item.product?.category_id || '')
