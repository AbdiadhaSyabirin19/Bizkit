...
export default function ProductPage() {
  ...
  const getPaginatedGroups = () => {
    ...
    pageItems.forEach((item, idx) => {
      ...
      result[catName].push({ ...item, _globalIndex: startIdx + idx + 1 })
    })
    return result
  }
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-[#cbd5e1] border-b">
                <th className="px-4 py-3 text-left text-xs font-bold text-gray-800 tracking-wide w-16">No</th>
                <th className="px-4 py-3 text-left text-xs font-bold text-gray-800 tracking-wide">Produk</th>
                <th className="px-4 py-3 text-left text-xs font-bold text-gray-800 tracking-wide">Kategori</th>
                <th className="px-4 py-3 text-right text-xs font-bold text-gray-800 tracking-wide">Harga Jual</th>
                <th className="px-4 py-3 text-center text-xs font-bold text-gray-800 tracking-wide w-24">Aksi</th>
              </tr>
            </thead>
            <tbody>
              {loading ? (
                ...
                Object.entries(paginatedGroups).map(([catName, items]) => (
                  <>
                    <tr key={`cat-${catName}`}>
                      <td colSpan={5} className="px-0 py-0">
                        <div className="flex items-center bg-gray-100 border-b">
                          <div className="w-1.5 h-8 bg-amber-500 rounded-r-sm"></div>
                    ...
                    {items.map((item) => (
                      <tr key={getID(item)} className="border-b hover:bg-gray-50 transition">
                        <td className="px-4 py-3 text-gray-600">{item._globalIndex}</td>
                        ...
                        <td className="px-4 py-3">
                          <div className="flex items-center justify-center gap-3">
                            <button
                              onClick={() => navigate(`/products/edit/${getID(item)}`)}
